...
from data_process.span_dataloader import SpanDataset
from torch.utils.data import DataLoader
from utils.utils import span_extract_item, load_data
...
def data_generator(tokenizer):
    ...
    predict_dataloader = DataLoader(SpanDataset(predict_data), batch_size=32, shuffle=False, drop_last=False,
...
def predict(dataloader, model, device, tokenizer):
    ...
    for batch_data in dataloader:
        ...
        span_res = span_extract_item(start_logits, end_logits)
        for ind in range(len(batch_sample)):
            text = batch_sample[ind]
            sample_res = span_res[ind]
            if not sample_res:
                continue
